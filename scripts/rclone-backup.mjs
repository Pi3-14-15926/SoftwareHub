/**
 * rclone 备份脚本 —— 下载一个同步一个模式
 *
 * 功能：
 *  1. 读取项目数据（新模型：page/{slug}/）
 *  2. 同步 GitHub Releases
 *  3. 逐个下载文件，每下载一个立即 rclone copy 到云盘
 *  4. 同步完成后删除本地临时文件
 *
 * 环境变量：
 *  RCLONE_REMOTE    — rclone 远程存储名称（如 "jianguoyun"）
 *  RCLONE_PATH      — 远程存储路径（默认 "/SoftwareHub"）
 *  GH_TOKEN         — GitHub 个人访问令牌（可选）
...
const RCLONE_PATH = (process.env.RCLONE_PATH || '/SoftwareHub').replace(/\/+$/, '')
...
async function main() {
  log('=== 开始 rclone 备份（下载一个同步一个模式） ===')

  // 检查 rclone 配置
  if (!RCLONE_REMOTE) {
    log('错误: 未设置 RCLONE_REMOTE 环境变量')
    process.exit(1)
  }

  log(`远程存储: ${RCLONE_REMOTE}`)
  log(`远程路径: ${RCLONE_PATH}`)
  ...
  for (const project of projects) {
    ...
    for (const ver of toBackup) {
      ...
      const versionDir = `${RCLONE_PATH}/${safeCategory}/${safeProject}/${dateStr}_${safeVer}`
      ...
      for (const dl of ver.downloads) {
        ...
        try {
          ...
          // 3. 立即同步到云盘
          log(`    ↑ 同步到 ${RCLONE_REMOTE}:${remoteFilePath}`)
          const syncResult = await rcloneCopyFile(localFilePath, remoteFilePath, (line) => {
